...
#Verifica se há ou nao o ip na interface
if [ $? -ne 0 ]; then
...
#Configura a rede 4.2.2.2 para o gateway padrão eth0
#----------------------------------------------------
...
if [ "$rotagoogle" = "$redeeth0" ]; then
        logger Botão ISH BOX WEB desativado - 3G desativado
elif [ "$rotagoogle" = "200.200.200.200" ]; then
        logger Botão ISH BOX WEB ativado - 3G ativado
else
...
if [ "$detectamodem" = "15cd" ];then
        logger Alterando o Drive para o Genérico
...
#Habilita o 3G para comunicar se não tiver o PPP0
if [ "$habilita3g" = "ppp0" ]; then
        logger Discagem já realizada no 3G
else
...
#Efetua a configuração das rotas: Firewall Martins, Google, Martins, Monitoramento
#----------------------------------------------------------------------------------
...
#Verifica se há ou nao retorno do ping
if [ $? -ne 0 ]; then
        ...
        if [ "$rotafirewalleth0" = "$redeeth1" ]; then
                        logger Rota Eth0 já adicionada
        else
...
#Efetua o teste de ping para o servidor da martins, se funcionar o ping ele muda o ipsec para o 3G, senão ele funciona pela eth0
#--------------------------------------------------------------------------------------------------------------------------------
...
#Verifica se funcionou ou não o ping
if [ $? -ne 0 ]; then
